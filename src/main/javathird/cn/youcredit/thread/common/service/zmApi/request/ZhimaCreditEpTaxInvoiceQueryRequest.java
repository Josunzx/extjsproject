...
import cn.youcredit.thread.common.service.zmApi.response.ZhimaCreditEpTaxInvoiceQueryResponse;

/**
 * ALIPAY API: zhima.credit.ep.tax.invoice.query request
 * 
 * @author auto create
 * @since 1.0, 2017-08-11 10:10:35
 */
public class ZhimaCreditEpTaxInvoiceQueryRequest implements ZhimaRequest<ZhimaCreditEpTaxInvoiceQueryResponse> {
	...
	* 商户传入的业务流水号。此字段由商户生成，需确保唯一性，用于定位每一次请求，后续按此流水进行对帐。生成规则: 固定30位数字串，前17位为精确到毫秒的时间yyyyMMddhhmmssSSS，后13位为自增数字。
	...
	public Class<ZhimaCreditEpTaxInvoiceQueryResponse> getResponseClass() {
		return ZhimaCreditEpTaxInvoiceQueryResponse.class;
	}
}
