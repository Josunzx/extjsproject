...
import cn.youcredit.thread.common.service.zmApi.response.ZhimaCreditEpIndividualScoreGetResponse;
 ...
 */
public class ZhimaCreditEpIndividualScoreGetRequest implements ZhimaRequest<ZhimaCreditEpIndividualScoreGetResponse> {
	...
	* transaction_id是代表一笔请求的唯一标志，该标识作为对账的关键信息，对于用户使用相同transaction_id的查询，芝麻在一天（86400秒）内返回首次查询数据，超过有效期的查询即为无效并返回异常（错误码xxxx），有效期内的重复查询不重新计费。 transaction_id 推荐生成方式是：30位，（其中17位时间值（精确到毫秒）：yyyyMMddHHmmssSSS）加上（13位自增数字：1234567890123）
	...
	public Class<ZhimaCreditEpIndividualScoreGetResponse> getResponseClass() {
		return ZhimaCreditEpIndividualScoreGetResponse.class;
	}
}
