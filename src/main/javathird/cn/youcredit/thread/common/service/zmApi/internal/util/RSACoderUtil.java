 ...
 * @version $Id: RSACoderUtil.java, v 0.1 2015年8月31日 下午3:16:16 xiaoqing.zhouxq
 *          Exp $
 */
public class RSACoderUtil extends CoderUtil {
     ...
     *         异常
     */
    public static String encrypt(String paramsString, String charset,
            String publicKey) throws Exception {
        byte[] encryptedResult = RSACoderUtil.encryptByPublicKey(
            paramsString.getBytes(charset), publicKey, null);
     ...
     */
    public static String encrypt(String paramsString, String charset,
            String publicKey, EncryptionModeEnum encryptionType)
            throws Exception {
        byte[] encryptedResult = RSACoderUtil.encryptByPublicKey(
            paramsString.getBytes(charset), publicKey, encryptionType);
     ...
     *         异常
     */
    public static String sign(SignTypeEnum signType, String data,
            String charset, String privateKey) throws Exception {
        byte[] dataInBytes = data.getBytes(charset);
        String signParams = RSACoderUtil
            .sign(signType, dataInBytes, privateKey);//用应用的私钥加签.
        return signParams;
    }
    ...
    public static String decrypt(String data, String key, String charset)
        ...
        byte[] encryptedBytes = RSACoderUtil.decryptByPrivateKey(byte64, key,
            null);
    ...
    public static byte[] decryptByPrivateKey(byte[] data, String key,
        ...
        KeyFactory keyFactory = KeyFactory
            .getInstance(RSACoderUtil.KEY_ALGORTHM);
        Key privateKey = keyFactory.generatePrivate(pkcs8EncodedKeySpec);
        //对数据解密
        Cipher cipher = Cipher.getInstance(RSACoderUtil.SPECIFIC_KEY_ALGORITHM);
        cipher.init(Cipher.DECRYPT_MODE, privateKey);

        // 解密时超过maxDecryptBlockSize字节就报错。为此采用分段解密的办法来解密
        int maxDecryptBlockSize;
        if (encryptionType != null) {
            maxDecryptBlockSize = RSACoderUtil
                .getMaxDecryptBlockSizeByEncryptionType(encryptionType);
        } else {
            maxDecryptBlockSize = RSACoderUtil.getMaxDecryptBlockSize(
    ...
    public static byte[] encryptByPublicKey(byte[] data, String key,
        ...
        if (encryptionType != null) {
            maxEncryptBlockSize = RSACoderUtil
                .getMaxEncryptBlockSizeByEncryptionType(encryptionType);
        } else {
            maxEncryptBlockSize = RSACoderUtil.getMaxEncryptBlockSize(
        ...
        try {
            ...
            for (int i = 0; i < data.length; i += maxEncryptBlockSize) {
                int encryptLength = dataLength - i < maxEncryptBlockSize ? dataLength
                    - i
                    : maxEncryptBlockSize;
    ...
    public static String sign(byte[] data, String privateKey) throws Exception {
        //解密私钥
        return RSACoderUtil.sign(SignTypeEnum.SHA1WITHRSA, data, privateKey);
    }
     ...
     *         异常
     */
    public static String sign(SignTypeEnum signType, byte[] data,
    ...
    public static boolean verify(byte[] data, String publicKey, String sign)
            throws Exception {
        return RSACoderUtil.verify(SignTypeEnum.SHA1WITHRSA, data, publicKey,
            sign);
    }
     ...
     *         异常
     */
    public static boolean verify(SignTypeEnum signType, byte[] data,
     ...
     * {
     * "encrypted":false,
     * "biz_response":"{"success":false,"error_code":"ZMOP.unknow_error","
     * error_message":"未知错误"}"
     * }
    ...
    public static String decryptResponse(String fullResponse,
        ...
        Map rootJson = RSACoderUtil.parseResponseMap(fullResponse);
        ...
        if (((Boolean) rootJson.get("encrypted")).booleanValue()) {
            decryptedRsp = RSACoderUtil.decrypt(decryptedRsp, privateKey,
                charset, encryptionType);
        }
        return decryptedRsp;
    }
    ...
    public static void verifySign(String fullResponse,
            ...
            throws Exception {
        RSACoderUtil.verifySign(SignTypeEnum.SHA1WITHRSA, fullResponse,
    ...
    public static void verifySign(SignTypeEnum signType, String fullResponse,
        ...
        if (sign != null && sign.length() > 0) {//在sign对象不为空的情况下才进行验签
            boolean success = RSACoderUtil.verify(signType,
    ...
    private static int getMaxEncryptBlockSize(KeyFactory keyFactory, Key key) {
        ...
        int maxLength = 117;
        try {
            RSAPublicKeySpec publicKeySpec = keyFactory.getKeySpec(key,
    ...
    private static int getMaxDecryptBlockSize(KeyFactory keyFactory, Key key) {
        ...
        int maxLength = 128;
        try {
            RSAPrivateKeySpec publicKeySpec = keyFactory.getKeySpec(key,
