...
public class Converters {
    ...
    static {
        Converters.baseFields.add("errorCode");
        Converters.baseFields.add("errorMessage");
        Converters.baseFields.add("body");
        Converters.baseFields.add("params");
        Converters.baseFields.add("success");
    }

    static {
        Converters.overideFields.add("success");
    }
    ...
    public static <T> T convert(Class<T> clazz, Reader reader)
        ...
        try {
            ...
            boolean isResponseClazz = ZhimaResponse.class
            ...
            for (PropertyDescriptor pd : pds) {
                ...
                // 之前errorCode的属性要剔除掉
                if (isResponseClazz
                    && Converters.excludeFields.contains(itemName)) {

                    continue;
                }

                List<ZhimaFieldMethod> alipayFieldMethods = new ArrayList<>();

                if (Converters.baseFields.contains(itemName) && isResponseClazz) {

                    Field field = ZhimaResponse.class
                    ...
                    if (writeMethod.getDeclaringClass().getName()
                        .contains("ZhimaResponse")) {

                        alipayFieldMethod.setMethod(writeMethod);
                    } else {
                        // 否则从父类再取一次
                        writeMethod = Converters.tryGetSetMethod(
                            ZhimaResponse.class, field, writeMethod.getName());
                        if (writeMethod == null) {
                            continue;
                        }
                    ...
                    // 如果被子类覆盖的，则从尝试从子类中获取
                    if (Converters.overideFields.contains(itemName)) {

                        field = Converters.tryGetFieldWithoutExp(clazz,
                            itemName);
                        // 属性存在则需要重新从子类获取访问方法
                        if (field != null) {

                            writeMethod = Converters.tryGetSetMethod(clazz,
                                field, writeMethod.getName());
                            if (writeMethod == null) {
                                continue;
                            }
                ...
                for (ZhimaFieldMethod alipayFieldMethod : alipayFieldMethods) {
                    ...
                    if (!reader.hasReturnField(itemName)) {
                        if (listName == null
                            || !reader.hasReturnField(listName)) {
                    ...
                    if (String.class.isAssignableFrom(typeClass)) {
                        ...
                        if (value instanceof String) {
                            ...
                            if (Converters.isCheckJsonType && value != null) {
                                throw new ZhimaApiException(itemName
                                    + " is not a String");
                            }
                            if (value != null) {
                                method.invoke(rsp, value.toString());
                        ...
                        if (value instanceof Long) {
                            ...
                            if (StringUtils.isNumeric(value)) {
                                method.invoke(rsp, Long
                                    .valueOf(value == null ? "null" : value
                                        .toString()));
                            }
                        }
                        ...
                        format.setTimeZone(TimeZone
                            .getTimeZone(ZhimaConstants.DATE_TIMEZONE));
                        Object value = reader.getPrimitiveObject(itemName);
                        if (value instanceof String) {
     ...
     * @return
     */
    private static <T> Method tryGetSetMethod(Class<T> clazz, Field field,
            String methodName) {
